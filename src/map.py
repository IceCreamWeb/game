...
class MapManager :
	...
	def __init__(self, screen, player):
		self.maps = dict() #"house" -> Map("house")
		self.screen = screen
		self.player = player
		...
		self.register_map("house", 
			portals=[Portal(from_world="house", origin_point="exit_house", target_world="world", teleport_point="enter_house_exit")
		])

		self.register_map("house2", 
			portals=[Portal(from_world="house2", origin_point="exit_house", target_world="world", teleport_point="exit_house2")
		])

		self.register_map("dungeon", portals=[
			Portal(from_world="dungeon", origin_point="exit_dungeon", target_world="world", tltport_point="dungeon_exit_spawn")
		])
	...
	def check_collisions(self):
		#portal
		for portal in self.get_map().portals: if portal.from_world
